...
def cumple_min_notional(symbol, precio, cantidad):
    """
    Verifica si el valor total de una orden cumple con el mínimo requerido por Binance (NOTIONAL).
    """
    try:
...
def calcular_cantidad_valida(symbol, precio_actual):
    """
    Calcula una cantidad válida de compra que cumple con los filtros de Binance:
    NOTIONAL, LOT_SIZE y MIN_QTY.
    """
    try:
        info = client.get_symbol_info(symbol)
        min_notional = None
        ...
        # Fallback manual para BTC si no se encuentra min_notional
        ...
        # Calcular cantidad con factor definido en PARAMS
        ...
        if total < min_notional:
            logging.warning(f"[{symbol}] Total insuficiente tras redondeo: {total} < mínimo {min_notional}")
            return None

        logging.info(f"[{symbol}] Cantidad válida calculada: {cantidad} | Total: {total} USDT")
        return cantidad

    except Exception as e:
        logging.error(f"[{symbol}] Error al calcular cantidad válida: {e}")
        return None